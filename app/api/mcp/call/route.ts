...
import { on3ToolsList, on3Call, on3ServiceToken } from '@/lib/on3'
...
export async function POST(req: NextRequest) {
  ...
  const svc = on3ServiceToken()
  if (!svc) return NextResponse.json({ ok: false, error: 'ON3_TOKEN is not configured.' }, { status: 500 })
  if (!tool) {
    const tools = await on3ToolsList(svc)
  ...
  const r = await on3Call(svc, tool, args)
  if (!r.ok) return NextResponse.json({ ok: false, error: r.error || 'The tool failed.' }, { status: 502 })
  return NextResponse.json({ ok: true, tool, text: r.text, ...(r.data !== undefined ? { data: r.data } : {}) })
}
