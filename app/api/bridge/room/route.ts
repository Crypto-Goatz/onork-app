...
async function peer(req: NextRequest) {
  ...
  if (!ctx) return { ok: false as const, why: 'That 0n key is not valid, or has been revoked.' }
  ...
  return { ok: true as const, db, name: data.name, userId: ctx.userId }
}
...
export async function POST(req: NextRequest) {
  ...
  if (typeof body.join === 'string' && body.join.trim()) {
    ...
    await db.from('bridge_peers').upsert(
      { room: ROOM, name, user_id: ctx.userId, last_seen: new Date().toISOString() },
      { onConflict: 'room,name' },
    )

    const { data: roster } = await db.from('bridge_peers').select('name').eq('room', ROOM)
    return NextResponse.json({
      joined: name,
  ...
  const { data: top } = await me.db
    .from('bridge_messages').select('seq').eq('room', ROOM)
  ...
  if (to) {
    const { data: exists } = await me.db
      .from('bridge_peers').select('name').eq('room', ROOM).eq('name', to).maybeSingle()
  ...
  return NextResponse.json({ posted: true, seq, from: me.name, to: to ?? 'everyone' })
}
