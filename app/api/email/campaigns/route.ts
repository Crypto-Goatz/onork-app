...
export async function GET(req: NextRequest) {
  ...
  const res = await fetch(`${CRM_API}/emails/schedule?locationId=${locationId}&campaignsOnly=true&showStats=true&limit=20`, {
    headers: { Authorization: `Bearer ${pit}`, Version: CRM_VERSION },
  })

  if (!res.ok) return NextResponse.json({ campaigns: [] })
  const data = await res.json()
  return NextResponse.json({ campaigns: data.schedules || [] })
}
