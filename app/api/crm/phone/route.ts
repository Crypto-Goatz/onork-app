...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (action === 'search') {
      const areaCode = searchParams.get('areaCode') || '412'
      const res = await crmGet(`/phone-number/search?type=local&areaCode=${areaCode}&limit=10`, locationId)
      if (!res.ok) {
        const text = await res.text()
        return NextResponse.json({ error: `CRM error: ${res.status}`, details: text }, { status: res.status })
      }
      const data = await res.json()
      return NextResponse.json({ numbers: data.data || data.numbers || [] })
    }

    const res = await crmGet('/phone-number/list', locationId)
    if (!res.ok) {
      const text = await res.text()
      return NextResponse.json({ error: `CRM error: ${res.status}`, details: text }, { status: res.status })
    }
    const data = await res.json()
    return NextResponse.json({ numbers: data.data || data.phoneNumbers || [] })
  } catch (error) {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { phoneNumber, forwardTo } = body

    if (!phoneNumber) {
      return NextResponse.json({ error: 'phoneNumber is required' }, { status: 400 })
    }

    const res = await crmPost('/phone-number/purchase', locationId, {
      phoneNumber,
