...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const res = await crmGet(`/funnels/?locationId=${locationId}&limit=${limit}`, locationId)
    if (!res.ok) {
      const text = await res.text()
      return NextResponse.json({ error: `CRM error: ${res.status}`, details: text }, { status: res.status })
    }
    const data = await res.json()
    return NextResponse.json({ funnels: data.funnels || [] })
  } catch (error) {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const res = await crmPost('/funnels/redirect', locationId, {
      locationId,
      source,
      target,
      type: type || '301',
