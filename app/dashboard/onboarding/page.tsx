    ...
    // Step 0: Welcome
    {
      ...
      content: (
        <div style={{ textAlign: 'center', padding: '20px 0' }}>
          ...
          <p style={{ color: '#8b9ab5', fontSize: 15, lineHeight: 1.7, maxWidth: 400, margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 8 }}>
          {INDUSTRIES.map(ind => (
            <button
              ...
              style={{
                padding: '12px 16px',
                background: data.industry === ind ? 'rgba(45,212,191,0.12)' : '#141e30',
                border: `1px solid ${data.industry === ind ? '#2dd4bf' : '#1c2b42'}`,
                borderRadius: 10,
                color: data.industry === ind ? '#2dd4bf' : '#8b9ab5',
                fontSize: 13,
      ...
      content: (
        <div>
          <textarea
            ...
            onBlur={e => e.target.style.borderColor = '#1c2b42'}
          />
          <p style={{ fontSize: 11, color: '#556880', marginTop: 8 }}>Separate with commas</p>
        </div>
      ),
      ...
      content: (
        <div style={{ textAlign: 'center' }}>
          ...
          <input
            ...
            onBlur={e => e.target.style.borderColor = '#1c2b42'}
          />
          <p style={{ fontSize: 12, color: '#556880', marginTop: 12 }}>
    ...
    // Step 6: Done
    {
      ...
      content: (
        <div style={{ textAlign: 'center', padding: '20px 0' }}>
          ...
          <div style={{ fontSize: 14, color: '#8b9ab5', lineHeight: 1.7, maxWidth: 360, margin: '0 auto' }}>
            <p><strong style={{ color: '#e8ecf2' }}>{data.businessName}</strong> · {data.industry}</p>
            <p style={{ marginTop: 8 }}>{data.services}</p>
            {data.phone && <p style={{ marginTop: 8 }}>{data.phone}</p>}
            <p style={{ marginTop: 12, color: '#2dd4bf', fontWeight: 600 }}>
  ...
  return (
    <div style={{
      ...
      <div style={{ width: '100%', maxWidth: 480 }}>
        {/* Progress bar */}
        <div style={{
          height: 3, background: '#1c2b42', borderRadius: 2,
        ...
        <div style={{ marginBottom: 32 }}>
          <h1 style={{ fontSize: 24, fontWeight: 700, color: '#e8ecf2', marginBottom: 6 }}>
            {current.title}
          </h1>
          <p style={{ fontSize: 14, color: '#556880' }}>{current.subtitle}</p>
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          {step > 0 ? (
            <button onClick={() => setStep(step - 1)} style={{
              ...
              color: '#8b9ab5', fontSize: 14, cursor: 'pointer',
            }}>Back</button>
          ) : <div />}
          ...
          {isLast ? (
            ...
            <button
              ...
              style={{
                padding: '12px 28px',
                background: current.valid ? 'linear-gradient(135deg, #2dd4bf, #14b8a6)' : '#1c2b42',
                color: current.valid ? '#0c1220' : '#556880',
        ...
        <div style={{
          ...
          {steps.map((_, i) => (
            <div key={i} style={{
              ...
              borderRadius: 4,
              background: i <= step ? '#2dd4bf' : '#1c2b42',
              transition: 'all 0.3s',
            }} />
          ))}
        </div>
