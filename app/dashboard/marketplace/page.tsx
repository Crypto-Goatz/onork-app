...
export default function MarketplaceDashboard() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: embedded ? '#f9fafb' : '#0c1220', color: embedded ? '#111827' : '#e8ecf2', fontFamily: 'system-ui, -apple-system, sans-serif', padding: embedded ? 16 : 24 }}>
      <div style={{ maxWidth: 1000, margin: '0 auto' }}>

        {/* Header */}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, paddingBottom: 12, borderBottom: `1px solid ${embedded ? '#e5e7eb' : '#1c2b42'}` }}>
          <div>
            <h1 style={{ fontSize: 20, fontWeight: 900, margin: 0 }}>
              <span style={{ color: '#10b981' }}>0n</span>MCP
              <span style={{ color: embedded ? '#9ca3af' : '#556880', fontSize: 12, fontWeight: 400, marginLeft: 8 }}>Dashboard</span>
            </h1>
            {sso && <p style={{ fontSize: 11, color: embedded ? '#9ca3af' : '#556880', marginTop: 2 }}>{sso.email} · Location: {sso.locationId?.slice(0, 8)}...</p>}
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            <a href="https://0nmcp.com" target="_blank" rel="noopener noreferrer" style={{ padding: '6px 14px', background: embedded ? '#f3f4f6' : '#141e30', border: `1px solid ${embedded ? '#e5e7eb' : '#1c2b42'}`, borderRadius: 8, color: embedded ? '#6b7280' : '#8b9ab5', fontSize: 11, textDecoration: 'none' }}>Docs</a>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 20 }}>
          {[
            ...
            <div key={s.label} style={{ background: embedded ? 'white' : '#141e30', border: `1px solid ${embedded ? '#e5e7eb' : '#1c2b42'}`, borderRadius: 12, padding: '14px 16px', textAlign: 'center' }}>
              <div style={{ fontSize: 24, fontWeight: 800, color: s.color }}>{s.value}</div>
              <div style={{ fontSize: 10, color: embedded ? '#9ca3af' : '#556880', marginTop: 2 }}>{s.label}</div>
        ...
        {/* Credits Bar */}
        <div style={{ background: embedded ? 'white' : '#141e30', border: `1px solid ${embedded ? '#e5e7eb' : '#1c2b42'}`, borderRadius: 12, padding: '14px 18px', marginBottom: 20 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: embedded ? '#6b7280' : '#556880', marginBottom: 6 }}>
            <span>AI Credits Used: {creditsUsed.toLocaleString()} / {credits.toLocaleString()}</span>
            <span>{creditsPercent}%</span>
          </div>
          <div style={{ height: 6, background: embedded ? '#f3f4f6' : '#1c2b42', borderRadius: 3, overflow: 'hidden' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
              {[
                ...
                <button key={a.label} style={{
                  background: embedded ? 'white' : '#141e30', border: `1px solid ${embedded ? '#e5e7eb' : '#1c2b42'}`,
                  borderRadius: 10, padding: '12px', textAlign: 'left', cursor: 'pointer',
                  transition: 'border-color 0.2s',
                }}>
                  <div style={{ fontSize: 20, marginBottom: 4 }}>{a.icon}</div>
                  <div style={{ fontSize: 12, fontWeight: 700, color: embedded ? '#111827' : '#e8ecf2' }}>{a.label}</div>
                  <div style={{ fontSize: 10, color: embedded ? '#9ca3af' : '#556880' }}>{a.desc}</div>
                </button>
              ))}
            </div>
          </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {recentActions.map((a, i) => (
                <div key={i} style={{
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 12, fontWeight: 600, color: embedded ? '#111827' : '#e8ecf2' }}>{a.action}</div>
                    <div style={{ fontSize: 10, color: embedded ? '#9ca3af' : '#556880' }}>{a.detail}</div>
                  </div>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
              {['Stripe', 'Slack', 'Gmail', 'Supabase', 'GitHub', 'SendGrid', 'Twilio', 'Shopify', 'Figma', 'Notion', '+85 more'].map(s => (
                <span key={s} style={{
                  ...
                  background: embedded ? '#f3f4f6' : '#1c2b42',
                  color: s.startsWith('+') ? '#10b981' : (embedded ? '#6b7280' : '#8b9ab5'),
                }}>
                  {s}
                </span>
              ))}
        ...
        {/* Footer */}
        <div style={{ textAlign: 'center', marginTop: 24, paddingTop: 16, borderTop: `1px solid ${embedded ? '#e5e7eb' : '#1c2b42'}`, fontSize: 10, color: embedded ? '#d1d5db' : '#3a4a5c' }}>
