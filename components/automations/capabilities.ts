...
export const CAPABILITIES: Capability[] = [
  ...
  },
  {
    ...
    name: 'Scheduled Time',
    description: 'Run at a specific time — daily, weekly, monthly, or custom cron',
    category: 'triggers',
    icon: 'CalendarClock',
    color: '#ef4444',
    steps: ['Fire at scheduled time', 'Pass timestamp context', 'Execute workflow'],
    configFields: [
      { key: 'schedule_type', label: 'How often?', type: 'select', options: ['Once', 'Daily', 'Weekly', 'Monthly', 'Custom Cron'], default: 'Daily', required: true },
      { key: 'time', label: 'What time? (ET)', type: 'text', placeholder: '09:00', default: '09:00' },
      { key: 'days', label: 'Which days? (for weekly)', type: 'text', placeholder: 'Mon,Wed,Fri' },
      { key: 'cron', label: 'Cron expression (for custom)', type: 'text', placeholder: '0 9 * * 1-5' },
    ],
  },
  {
    id: 'trigger_recurring',
    name: 'Recurring Interval',
    description: 'Run every X minutes or hours — polling style automation',
    category: 'triggers',
    icon: 'RefreshCw',
    color: '#ef4444',
    steps: ['Set up recurring timer', 'Fire at each interval', 'Execute workflow'],
    configFields: [
      { key: 'interval_value', label: 'Every', type: 'number', placeholder: '60', required: true },
      { key: 'interval_unit', label: 'Unit', type: 'select', options: ['minutes', 'hours'], default: 'minutes' },
    ],
  },
  {
    id: 'trigger_webhook',
    name: 'Inbound Webhook',
    description: 'Starts when an external system POSTs to your unique webhook URL',
    category: 'triggers',
    icon: 'Webhook',
    color: '#ef4444',
    steps: ['Listen for POST to unique webhook URL', 'Parse JSON body', 'Pass data to next step'],
    configFields: [
      { key: 'webhook_name', label: 'Name this webhook', type: 'text', placeholder: 'stripe-payment', required: true },
    ],
  },
  {
    id: 'trigger_contact_updated',
    name: 'Contact Updated',
    description: 'Starts when any contact field changes in the CRM',
    category: 'triggers',
    icon: 'UserPen',
    color: '#ef4444',
    steps: ['Watch CRM ContactUpdate event', 'Identify changed fields', 'Pass old + new values'],
  },
  {
    id: 'trigger_opportunity_stage',
    name: 'Deal Stage Changed',
    description: 'Starts when an opportunity moves to a new pipeline stage',
    category: 'triggers',
    icon: 'ArrowRightLeft',
    color: '#ef4444',
    steps: ['Watch CRM OpportunityStatusUpdate', 'Identify new stage', 'Pass opportunity data'],
    configFields: [
      { key: 'target_stage', label: 'Stage name (blank = any)', type: 'text', placeholder: 'Closed Won' },
    ],
  },
  {
    id: 'trigger_conversation_reply',
    name: 'New Message Received',
    description: 'Starts when a contact sends a new email, SMS, or chat message',
    category: 'triggers',
    icon: 'MessageCircle',
    color: '#ef4444',
    steps: ['Watch CRM conversation events', 'Extract message content', 'Identify contact'],
  },
  {
    id: 'trigger_email_opened',
    name: 'Email Opened',
    description: 'Starts when a contact opens an email you sent',
    category: 'triggers',
    icon: 'MailOpen',
    color: '#ef4444',
    steps: ['Watch email tracking', 'Identify contact', 'Log open event'],
  },
  {
    id: 'trigger_stripe_event',
    name: 'Stripe Event',
    description: 'Starts on any Stripe event — payment succeeded, subscription created, invoice paid, etc.',
    category: 'triggers',
    icon: 'CreditCard',
    color: '#ef4444',
    steps: ['Watch Stripe webhook', 'Parse event type', 'Extract customer + amount data'],
    configFields: [
      { key: 'event_type', label: 'Which event?', type: 'select', options: ['payment_intent.succeeded', 'customer.subscription.created', 'customer.subscription.deleted', 'invoice.paid', 'invoice.payment_failed', 'any'], default: 'any' },
    ],
  },
  {
    id: 'trigger_wordpress_event',
    name: 'WordPress Event',
    description: 'Starts when something happens on a connected WordPress site',
    category: 'triggers',
    icon: 'Globe',
    color: '#ef4444',
    steps: ['Watch WordPress webhook', 'Parse event', 'Pass data to workflow'],
    configFields: [
      { key: 'wp_event', label: 'Which event?', type: 'select', options: ['post_published', 'comment_added', 'user_registered', 'form_submitted', 'woocommerce_order', 'any'], default: 'any' },
    ],
  },
  {
    id: 'trigger_market_match',
    name: 'New Market Match',
    description: 'Starts when Market Intel finds a new high-score opportunity',
    category: 'triggers',
    icon: 'TrendingUp',
    color: '#ef4444',
    steps: ['Watch for new matches above threshold', 'Extract opportunity details', 'Pass to workflow'],
    configFields: [
      { key: 'min_score', label: 'Minimum match score', type: 'number', placeholder: '70', default: '70' },
    ],
  },
