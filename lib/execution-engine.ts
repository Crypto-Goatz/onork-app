...
async function executeStep(
  ...
  try {
    ...
    switch (action) {
      ...
      case 'heygen.create_video':
      case 'heygen.create_video_agent': {
        const hgCreds = await getUserServiceCreds(ctx.userId, 'heygen')
        if (!hgCreds?.api_key) throw new Error('HeyGen not connected')
        const res = await fetch('https://api.heygen.com/v2/video/generate', {
          method: 'POST',
          headers: { 'X-Api-Key': hgCreds.api_key, 'Content-Type': 'application/json' },
          body: JSON.stringify({
            video_inputs: [{
              character: { type: 'avatar', avatar_id: (input.avatar_id as string) || 'default' },
              voice: { type: 'text', input_text: (input.script as string) || (input.text as string) || '' },
            }],
      ...
      case 'heygen.create_avatar_video': {
        const hgCreds = await getUserServiceCreds(ctx.userId, 'heygen')
        if (!hgCreds?.api_key) throw new Error('HeyGen not connected')
        const res = await fetch('https://api.heygen.com/v2/video/generate', {
          method: 'POST',
          ...
          body: JSON.stringify({
            video_inputs: [{
              character: {
                type: 'avatar',
                avatar_id: (input.avatar_id as string) || 'default',
                avatar_style: (input.avatar_style as string) || 'normal',
              },
              voice: {
                type: 'text',
                input_text: (input.script as string) || '',
                voice_id: (input.voice_id as string) || undefined,
              },
              background: input.background ? { type: 'color', value: input.background as string } : undefined,
            }],
            dimension: { width: (input.width as number) || 1920, height: (input.height as number) || 1080 },
            title: (input.title as string) || 'Generated by 0nCore',
          }),
        })
        data = await res.json()
        break
      ...
      case 'heygen.list_videos': {
        const hgCreds = await getUserServiceCreds(ctx.userId, 'heygen')
        if (!hgCreds?.api_key) throw new Error('HeyGen not connected')
        const limit = (input.limit as number) || 10
        const res = await fetch(`https://api.heygen.com/v1/video.list?limit=${limit}`, {
          headers: { 'X-Api-Key': hgCreds.api_key },
        })
        data = await res.json()
        break
      }

      case 'heygen.get_video': {
        const hgCreds = await getUserServiceCreds(ctx.userId, 'heygen')
        if (!hgCreds?.api_key) throw new Error('HeyGen not connected')
        const videoId = input.video_id as string
        if (!videoId) throw new Error('video_id required')
      ...
      case 'heygen.translate_video': {
        ...
        const res = await fetch('https://api.heygen.com/v2/video_translate', {
          ...
          body: JSON.stringify({
            ...
            output_language: input.language as string,
          }),
        })
        data = await res.json()
        break
...
async function resolveIntent(command: string, ctx: UserContext): Promise<unknown> {
  ...
  if (lower.match(/(?:create|make|generate)\s+(?:a\s+)?(?:video|avatar)/)) {
    return { action: 'heygen.create_video', parsed: { text: command }, message: 'Use heygen.create_video with a script to generate a video.' }
  }

  if (lower.match(/(?:list|show)\s+(?:my\s+)?(?:videos|heygen)/)) {
    return { action: 'heygen.list_videos', parsed: {} }
  }
